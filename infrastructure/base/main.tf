locals {
  ...
  db_instance_name = var.db_instance_name != "" ? var.db_instance_name : "mc-cluster-${var.cluster_uuid}"
  database_name    = var.database_name != "" ? var.database_name : "cluster_${local.cluster_uuid_underscore}"
...
resource "google_storage_bucket_object" "init_script" {
  ...
  content = templatefile("${path.module}/scripts/init_database.sh", {
    ...
    db_user     = google_sql_user.admin.name
