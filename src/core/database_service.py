...
class DatabaseService:
    ...
    def create_sub_hospital_database(self, parent_uuid: str, sub_hospital_name: str, sub_hospital_uuid: str) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            logger.info(f"Creating database '{db_name}' in parent MySQL instance {host}:{port}")
    ...
    def create_tables(self, client_uuid: str, is_sub_hospital: bool = False, region: str = None, private_bucket_name: str = None, parent_uuid: str = None, database_name: str = None) -> Tuple[bool, str]:
        """
...
if timeout 30 mysql -h {host} -P {port} -u {user} -p'{escaped_password}' --connect-timeout=10 -e "SELECT 1 as connection_test;" {database} 2>&1; then
...
timeout 540 mysql -h {host} -P {port} -u {user} -p'{escaped_password}' --connect-timeout=10 {database} < /tmp/create_tables_{client_uuid}.sql 2>&1
